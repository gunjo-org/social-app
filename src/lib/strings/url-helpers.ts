...
export const BSKY_APP_HOST = 'https://app.gunjo.org'
...
export function toShareUrl(url: string): string {
  if (!url.startsWith('https')) {
    const urlp = new URL('https://app.gunjo.org')
    urlp.pathname = url
    url = urlp.toString()
  }
  return url
...
export function isBskyAppUrl(url: string): boolean {
  return (
    ...
    (url.startsWith('https://app.gunjo.org/') &&
      !url.startsWith('https://app.gunjo.org/about'))
  )
}
